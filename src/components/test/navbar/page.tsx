            ...
            <nav className="w-full h-20 flex items-center gap-8 px-10 bg-white fixed">
                ...
                <div className=" md:flex hidden items-center ml-auto gap-8">
                    {NAVBAR_PAGES.map((item, index) => (
                        <Link className=" text-green-900 font-semibold" key={index} href={item.href}> {item.page} </Link>
                    ))}
                    <Link href={"/ContactUs"}>
                        <button className="px-4 hover:bg-green-800 font-semibold py-2 rounded-md bg-green-900">  Contact Us  </button>
                    </Link>
                </div>
                <button onClick={toggleMenu} className={`md:hidden ml-auto duration-200 text-green-900 ${IsOpen ? "rotate-0" : "rotate-90"}`}>

                    {IsOpen ? < LucideMenu /> : < X className="rotate:45" />}
                    {/* <LucideMenu className="md:hidden ml-auto text-green-900 flex "></LucideMenu> */}
                </button>
            </nav>
        </main >
    )
