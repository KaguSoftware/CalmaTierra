            ...
            <nav className="w-full h-20 flex items-center gap-8 px-10 bg-white fixed">
                ...
                <div className=" md:flex hidden items-center ml-auto gap-8">
                    {NAVBAR_PAGES.map((item, index) => (
                        <Link className=" text-green-900 font-semibold" key={index} href={item.href}>
                            {item.page}
                        </Link>
                    ))}
                    <Link href={"/ContactUs"}>
                        <button className="px-4 hover:bg-green-800 font-semibold py-2 rounded-md bg-green-900">  Contact Us  </button>
                    </Link>
                </div>

                <button onClick={toggleMenu} className={`md:hidden ml-auto duration-200 relative text-green-900 ${IsOpen ? "rotate-0" : "rotate-90"}`}>
                    ...
                    <div className="bg-amber-400">
                        {NAVBAR_PAGES.map((item, index) => (
                            ...
                            <div className="text-bold text-xl px-6 py-6 bg-white h-screen w-screen  flex flex-col gap-4">
                                ...
                                <Link className=" text-green-900 font-semibold" onClick={() => SetIsOpen(false)} key={index} href={item.href}>
                                    {item.page}
                                </Link>
                            </div>
                        ))}
